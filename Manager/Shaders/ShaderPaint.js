...
export const shaderPaint = (uniforms) => {
  const material = new ShaderMaterial({
    ...
    vertexShader: monkeyPatch(ShaderChunk.meshphysical_vert, {
      header: `
        ...
        float displace(vec3 point) {
          ...
          return noise(vec3(point.x * frequency + vel, point.y * frequency - vel, point.z * frequency + vel)) * amplitude * (1.0 - uv.y);
        }
    ...
    fragmentShader: monkeyPatch(ShaderChunk.meshphysical_frag, {
      ...
      '#include <dithering_fragment>': `
        float alphaM = smoothstep(0.0,0.3, vUv.y) - smoothstep(progress,progress+0.1, vUv.y);
